...
export const useFeedbackStore = defineStore('feedback', () => {
   ...
   */
  const getAllFeedback = async (force = false) => {
    if (!force && feedbackList.value.length && Date.now() - _feedbackCachedAt < CACHE_TTL) {
   ...
   */
  const getFeedbackStats = async (force = false) => {
    if (!force && stats.value && Date.now() - _statsCachedAt < CACHE_TTL) {
      return stats.value
    }
    try {
  ...
  const addFeedback = async (rating, comment) => {
    ...
    try {
      ...
      if (response.data.success) {
        // Refresh feedback list and stats (force bypasses cache; allSettled avoids crash on partial failure)
