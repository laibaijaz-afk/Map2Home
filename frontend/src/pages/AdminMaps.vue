  ...
  <div class="min-h-screen bg-gradient-to-br from-red-50 to-orange-50">
    ...
    <main class="max-w-7xl mx-auto px-6 py-12">
      ...
      <div class="mb-8">
        ...
        <p class="text-gray-600">Upload DXF maps with room specifications so users can find matching floor plans</p>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div class="bg-white rounded-lg shadow p-6">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-sm text-gray-600 mb-1">Avg Bedrooms</p>
              <p class="text-3xl font-bold text-gray-900">{{ avgBedrooms }}</p>
            </div>
          ...
          <table class="w-full">
            <thead class="bg-gray-50 border-b">
              <tr>
                ...
                <th class="px-4 py-3 text-center text-xs font-semibold text-gray-600 uppercase">Bed</th>
                <th class="px-4 py-3 text-center text-xs font-semibold text-gray-600 uppercase">Bath</th>
                <th class="px-4 py-3 text-center text-xs font-semibold text-gray-600 uppercase">Kitchen</th>
                <th class="px-4 py-3 text-center text-xs font-semibold text-gray-600 uppercase">Drawing</th>
                <th class="px-4 py-3 text-center text-xs font-semibold text-gray-600 uppercase">Garage</th>
                <th class="px-4 py-3 text-center text-xs font-semibold text-gray-600 uppercase">Plot</th>
            ...
            <tbody class="divide-y divide-gray-100">
              <tr v-for="map in maps" :key="map.id" class="hover:bg-gray-50 transition">
                ...
                <td class="px-4 py-3">
                  <span class="text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded font-mono">{{ map.file_path }}</span>
                </td>
                <td class="px-4 py-3 text-center font-medium">{{ map.bedrooms }}</td>
                <td class="px-4 py-3 text-center font-medium">{{ map.bathrooms }}</td>
                <td class="px-4 py-3 text-center font-medium">{{ map.kitchen }}</td>
                <td class="px-4 py-3 text-center font-medium">{{ map.drawing_room }}</td>
                <td class="px-4 py-3 text-center font-medium">{{ map.garage }}</td>
                <td class="px-4 py-3 text-center text-xs text-gray-600">
                  <span v-if="map.plot_length && map.plot_width">{{ map.plot_length }}x{{ map.plot_width }}</span>
    ...
    <div v-if="showAddModal || showEditModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" @click.self="closeModals">
      <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto mx-4">
        ...
        <form @submit.prevent="showEditModal ? updateMap() : addMap()" class="p-6 space-y-5">
          <!-- DXF File Upload (only for Add) -->
          <div v-if="!showEditModal">
            <label class="block text-sm font-semibold text-gray-700 mb-2">DXF File *</label>
            <div
              class="border-2 border-dashed rounded-lg p-6 text-center transition"
              :class="form.dxfFile ? 'border-green-400 bg-green-50' : 'border-gray-300 hover:border-blue-400'"
            >
              <input type="file" accept=".dxf" @change="handleFileSelect" class="hidden" ref="fileInput" />
              <div v-if="form.dxfFile" class="flex items-center justify-center gap-2">
                <svg class="w-5 h-5 text-green-600" fill="currentColor" viewBox="0 0 20 20">
                  <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" />
                </svg>
                <span class="font-medium text-green-700">{{ form.dxfFile.name }}</span>
                <button type="button" @click="form.dxfFile = null" class="text-red-500 hover:text-red-700 ml-2">
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                  </svg>
                </button>
              ...
              <div v-else>
                ...
                <button type="button" @click="$refs.fileInput.click()" class="text-blue-600 font-semibold hover:text-blue-700">
                  Click to upload a .dxf file
                </button>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div class="grid grid-cols-3 md:grid-cols-5 gap-3">
              <div v-for="spec in specFields" :key="spec.key">
                <label class="block text-xs font-medium text-gray-600 mb-1">{{ spec.label }}</label>
                <input v-model.number="form[spec.key]" type="number" min="0" placeholder="0"
                  class="w-full px-2 py-1.5 border border-gray-300 rounded-lg text-center focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm" />
              </div>
            </div>
          </div>
          ...
          <!-- Plot Dimensions -->
          <div>
            <h4 class="text-sm font-bold text-gray-700 mb-3">Plot Dimensions (optional)</h4>
            <div class="grid grid-cols-2 gap-4">
              <div>
                <label class="block text-xs font-medium text-gray-600 mb-1">Length (feet)</label>
                <input v-model.number="form.plot_length" type="number" min="0" step="0.1" placeholder="e.g. 30"
                  class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
              <div>
                <label class="block text-xs font-medium text-gray-600 mb-1">Width (feet)</label>
                <input v-model.number="form.plot_width" type="number" min="0" step="0.1" placeholder="e.g. 45"
                  class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
            </div>
          </div>
...
const specFields = [
  ...
  { key: 'servant_quarter', label: 'Servant Qtr' },
...
const avgBedrooms = computed(() => {
  if (maps.value.length === 0) return 0
  const total = maps.value.reduce((sum, m) => sum + (m.bedrooms || 0), 0)
  return (total / maps.value.length).toFixed(1)
})
...
const addMap = async () => {
  if (!form.value.dxfFile) {
    showAlert('error', 'Please select a DXF file')
    return
  }
  ...
  try {
    ...
    fd.append('dxfFile', form.value.dxfFile)
    fd.append('title', form.value.title)
    fd.append('description', form.value.description)
    fd.append('room_type', 'house')
    for (const spec of specFields) {
      fd.append(spec.key, form.value[spec.key] || 0)
    }
    if (form.value.plot_length) fd.append('plot_length', form.value.plot_length)
    if (form.value.plot_width) fd.append('plot_width', form.value.plot_width)
...
const updateMap = async () => {
  ...
  try {
    ...
    for (const spec of specFields) {
      body[spec.key] = form.value[spec.key] || 0
    }
    if (form.value.plot_length) body.plot_length = form.value.plot_length
    if (form.value.plot_width) body.plot_width = form.value.plot_width
