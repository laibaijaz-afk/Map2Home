...
const fetchMaterials = async () => {
  ...
  try {
    ...
    const response = await axios.get(`${apiUrl}/admin/materials`, {
      ...
      timeout: 8000 // fail fast so UI can show actionable error quickly
    })
    ...
    if (err.code === 'ECONNABORTED' || /timeout/i.test(err.message || '')) {
      error.value = 'Backend request timed out. Check that backend and MySQL are running, then retry.'
    } else if (err.message === 'Network Error' || /ERR_NETWORK/i.test(err.code || '')) {
      error.value = 'Cannot reach backend API. Verify backend is running on http://localhost:5001.'
    } else {
