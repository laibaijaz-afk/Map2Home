          ...
          <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-200">
              <tr v-for="material in filteredMaterials" :key="material.id" class="hover:bg-gray-50 transition">
                ...
                <td class="px-6 py-4">
                  <span class="text-lg font-bold text-gray-900">Rs {{ formatCurrency(material.cost_per_unit) }}</span>
                </td>
                <td class="px-6 py-4">
                  <div class="relative">
                    ...
                    <input
                      ...
                      class="w-36 pl-12 pr-3 py-2 border-2 border-gray-300 rounded-lg focus:outline-none focus:border-red-500 transition"
                    />
                  </div>
                </td>
                <td class="px-6 py-4">
                  <button
                    ...
                    :disabled="!material.newCost || material.newCost === material.cost_per_unit || savingId === material.id"
        ...
        <div class="flex items-start">
          ...
          <div>
            ...
            <ul class="text-sm text-blue-800 space-y-1 list-disc list-inside">
              <li>Enter the new cost in the "New Cost" field for any material</li>
              <li>Click the "Save" button to update the cost in the database</li>
              <li>Updated costs will be used immediately in cost estimation calculations</li>
              <li>All costs are in Rs (Pakistani Rupees) per unit</li>
            </ul>
          </div>
        </div>
      </div>
...
const fetchMaterials = async () => {
  ...
  try {
    ...
    materials.value = response.data.materials.map(m => ({
      ...m,
      newCost: null
    }))
    ...
    error.value = err.response?.data?.message || 'Failed to load materials'
...
const updateCost = async (material) => {
  if (!material.newCost || material.newCost === material.cost_per_unit) return
  ...
  try {
    ...
    // Update local data
    material.cost_per_unit = material.newCost
    ...
    alert.value = {
      show: true,
      type: 'success',
      message: `Cost for ${material.name} updated successfully to Rs ${formatCurrency(material.cost_per_unit)}`
    }
    ...
    alert.value = {
      show: true,
      type: 'error',
      message: err.response?.data?.message || 'Failed to update material cost'
    }
