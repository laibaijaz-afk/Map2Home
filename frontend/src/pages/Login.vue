...
// Redirect if already authenticated, or show session-expired message
onMounted(() => {
  if (authStore.isAuthenticated) {
    const target = authStore.isAdmin ? '/admin/materials' : '/'
    router.push(target)
    return
  }
...
const handleLogin = async () => {
  ...
  try {
    ...
    // Redirect to home page; use query.redirect if user was sent to login from a protected page.
...
const handleGoogleSignIn = async () => {
  ...
  try {
    alert.value = {
      ...
      type: 'info',
      title: 'Coming Soon',
      message: 'Google Sign-In is not available yet. Please use email and password to login.'
    }
  } finally {
