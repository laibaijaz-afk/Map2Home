      ...
      <div class="construction-card flex items-center justify-between gap-4 p-4 md:p-5 backdrop-blur-md">
        ...
        <div class="hidden md:flex flex-1 justify-center px-4">
          <ul class="flex items-center gap-2 text-sm font-semibold">
            ...
            <li>
              <RouterLink
                to="/user/design-2d-map"
                class="px-4 py-2 rounded-lg text-wood-700 hover:bg-wood-200/50 hover:text-construction-600 transition-all duration-200 flex items-center gap-2"
                :class="{ 'bg-wood-200/60 text-construction-700': route.path === '/user/design-2d-map' || route.path === '/dashboard' }"
              >
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <li>
              <RouterLink
                to="/cost-estimation"
                class="px-4 py-2 rounded-lg text-wood-700 hover:bg-wood-200/50 hover:text-construction-600 transition-all duration-200 flex items-center gap-2"
                :class="{ 'bg-wood-200/60 text-construction-700': route.path === '/cost-estimation' }"
              >
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <li>
              <RouterLink
                to="/material-prices"
                class="px-4 py-2 rounded-lg text-wood-700 hover:bg-wood-200/50 hover:text-construction-600 transition-all duration-200 flex items-center gap-2"
                :class="{ 'bg-wood-200/60 text-construction-700': route.path === '/material-prices' }"
              >
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="flex items-center gap-3">
              ...
              <div class="hidden sm:flex items-center gap-3">
                <template v-if="authStore.isAdmin">
                  ...
                  <RouterLink to="/admin/maps" class="text-dark hover:text-primary font-semibold">Maps</RouterLink>
    ...
    <transition name="fade">
      <div v-if="showMobile" class="md:hidden max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-3">
        <div class="bg-gradient-to-br from-wood-50 to-wood-100 border-2 border-wood-300/50 shadow-lg p-4 flex flex-col gap-3 rounded-xl">
          ...
          <RouterLink
            to="/user/design-2d-map"
            class="text-wood-700 hover:text-construction-600 hover:bg-wood-200/50 px-3 py-2 rounded-lg transition"
            :class="{ 'bg-wood-200/60 text-construction-700': route.path === '/user/design-2d-map' || route.path === '/dashboard' }"
          >Services</RouterLink>
          ...
          <div class="border-t border-white/20 mt-2 pt-3 flex flex-col gap-2">
            ...
            <template v-else>
              ...
              <RouterLink v-if="authStore.isAdmin" to="/admin/maps" class="text-wood-700 hover:text-construction-600 hover:bg-wood-200/50 px-3 py-2 rounded-lg transition font-semibold">Maps</RouterLink>
