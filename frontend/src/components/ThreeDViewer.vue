...
let canJump = true
const PLAYER_RADIUS = 12 // Slim collision body - fits through doorways, still blocks wall penetration

// Check if position (x,z) would collide with any wall (player as cylinder in XZ)
...
const createFloor = (bounds) => {
  const SPACE_EXPANSION = 1.35
  const width = (bounds.maxX - bounds.minX) * 1.3 * SPACE_EXPANSION
  const depth = (bounds.maxY - bounds.minY) * 1.3 * SPACE_EXPANSION
...
const createCeiling = (bounds) => {
  const SPACE_EXPANSION = 1.35
  const width = (bounds.maxX - bounds.minX) * 1.1 * SPACE_EXPANSION
  const depth = (bounds.maxY - bounds.minY) * 1.1 * SPACE_EXPANSION
  const centerX = (bounds.minX + bounds.maxX) / 2
  const centerZ = (bounds.minY + bounds.maxY) / 2
...
const createWall = (entity) => {
  ...
  // Wall thickness - thinner walls = more space, less congested feeling
  const thickness = Math.max(mapSize * 0.0015, 2) // Thinner for more open corridors
  const SPACE_EXPANSION = 1.35 // Expand layout for wider corridors
  ...
  wallTexture.repeat.set(scaledLength / 200, height / 200)

  const geometry = new THREE.BoxGeometry(scaledLength, height, thickness)
  geometry.computeBoundingBox() // Needed for wall collision detection in walk mode
  const material = new THREE.MeshStandardMaterial({
    map: wallTexture,
    roughness: 0.9,
    metalness: 0
  ...
  // Expand wall positions from map center for more spacious feel
...
// Add room labels on the floor (Bedroom, Kitchen, etc.)
const addFloorLabels = () => {
  if (!mapBounds || !props.mapData?.entities) return
  
  const rooms = detectRooms()
  const mapSize = Math.max(mapBounds.maxX - mapBounds.minX, mapBounds.maxY - mapBounds.minY)
  const labelWidth = Math.max(mapSize * 0.03, 30)
  const labelHeight = Math.max(mapSize * 0.012, 12)
  const labelY = 3 // Above floor to avoid z-fighting with floor texture
  ...
  rooms.forEach(room => {
    ...
    label.renderOrder = 10 // Draw on top so labels stay visible
...
const animate = () => {
  ...
  if (isWalkMode.value && isPointerLocked.value) {
    ...
    // Ground collision - snap to floor level, feel like walking on ground
