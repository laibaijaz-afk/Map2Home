...
const downloadHref = computed(() => blobObjectUrl.value || props.dxfUrl)
const downloadName = computed(() => {
  const base = (props.mapTitle || 'floor-plan').replace(/[^\w\-]+/g, '-').replace(/^-|-$/g, '') || 'floor-plan'
  return `${base}.dxf`
})
...
const loadDxf = async () => {
  ...
  try {
    ...
    const currentObjectUrl = blobObjectUrl.value
