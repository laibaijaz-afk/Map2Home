...
async function scrapeMaterialPage(url, materialKey) {
  try {
    ...
    if (materialKey === 'bricks') {
      const brickPatterns = [
        /(?:brick|bricks)[\s:]*[\s(]*(?:rs|pkr)?[\s:]*([\d,]+)[\s–-]?([\d,]+)?[\s]*per[\s]*(?:brick|piece)/gi,
        /([\d]{2})[\s]*(?:rs|pkr)?[\s]*(?:per|\/)?[\s]*(?:brick|piece)/gi,
        /(?:per|\/)\s*(?:brick|piece)[\s:]+(?:rs|pkr)?[\s:]*([\d]{2})/gi
      ];
    ...
    if (prices.length > 0) {
      ...
      if (validPrices.length === 0) {
        console.log(`[Scraper] ${materialKey}: No valid prices found (found ${prices.length} total prices)`);
        return null;
      }
