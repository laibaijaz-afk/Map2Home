  ...
  try {
    ...
    try {
      ...
      if (!e.message.includes('Duplicate column name')) {
        console.warn('Error adding source_url column:', e.message);
      }
    }
    ...
    try {
      ...
      // Column might already exist, ignore
      if (!e.message.includes('Duplicate column name')) {
        console.warn('Error adding last_scraped_at column:', e.message);
      }
    }

    // Check if materials exist, if not insert defaults
    ...
    if (existingMaterials[0].count === 0) {
      await connection.query(`
        ...
        ('Cement', 'construction', 1390.00, 'bag', 'Cement (Maple Leaf, DG, Bestway, etc.)'),
        ('Steel', 'construction', 238000.00, 'ton', 'Steel bars/Sariya for reinforcement'),
        ('Bricks', 'construction', 17.00, 'piece', 'Red bricks/Clay bricks'),
        ('Sand', 'construction', 10000.00, 'truck', 'Sand/Rait for construction'),
        ('Gravel', 'construction', 25500.00, 'truck', 'Crush/Bajri for construction'),
        ('Tiles', 'finishing', 400.00, 'sqm', 'Ceramic tiles'),
        ('Paint', 'finishing', 1250.00, 'liter', 'Paint (Brighto, Berger, Nippon, etc.)'),
        ('Aluminum', 'construction', 1450.00, 'sqft', 'Aluminum windows/doors'),
        ('Glass', 'finishing', 193.00, 'sqft', 'Tempered/Clear glass'),
        ('Electrical Wire', 'construction', 83.00, 'meter', 'Electric cable/wire'),
        ('Plumbing Pipe', 'construction', 463.00, 'meter', 'PPRC plumbing pipe')
      `);
    }
    ...
    let selectFields = 'id, name, category, cost_per_unit, unit, description, is_active, updated_at';
    if (hasSourceUrl) selectFields += ', source_url';
    if (hasLastScraped) selectFields += ', last_scraped_at';
    
    const [materials] = await connection.query(
      `SELECT ${selectFields} FROM materials WHERE is_active = TRUE ORDER BY category, name`
    );

    connection.release();

    console.log(`[Admin] Returning ${materials.length} materials`);
...
const updateMaterialCost = async (req, res, next) => {
  try {
    ...
    // Update material cost (manual update - doesn't change last_scraped_at)
    await connection.query(
      'UPDATE materials SET cost_per_unit = ?, updated_by = ?, updated_at = NOW() WHERE id = ?',
      [costPerUnit, req.user.id, materialId]
    );
    
    console.log(`[Admin] Material ${materialId} cost updated to ${costPerUnit} by user ${req.user.id}`);
