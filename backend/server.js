...
// Body Parser — must come BEFORE rate limiter so large bodies are consumed
// before any early response is sent (prevents ECONNRESET on client).
app.use(express.json({ limit: '50mb' }));
app.use(express.urlencoded({ extended: true, limit: '50mb' }));

// Rate limiting — applied AFTER body parser
const limiter = rateLimit({
  windowMs: 15 * 60 * 1000,
  max: 1000 // generous limit for development; tighten in production
});
...
(async () => {
  ...
  floorPlan3DRoutes = require('./routes/floorPlan3DRoutes');
  ...
  app.use('/api/floorplan-3d', floorPlan3DRoutes);
